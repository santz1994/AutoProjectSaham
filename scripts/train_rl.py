...
class GymWrapper:
    ...
    def __init__(self, prices: list[float]):
        ...
        try:
            ...
            self.action_space = getattr(self.inner, 'action_space', gym.spaces.MultiDiscrete([4, 5]))
            obs_shape = getattr(self.inner, 'observation_shape', (8,))
            self.observation_space = getattr(self.inner, 'observation_space', gym.spaces.Box(low=-1e9, high=1e9, shape=obs_shape, dtype=np.float32))
        except Exception:
...
def main(argv=None):
    ...
    parser.add_argument('--checkpoint-freq', type=int, default=0, help='checkpoint save frequency (steps). 0 -> auto')
    ...
    try:
        ...
        checkpoint_cb = CheckpointCallback(save_freq=cp_freq, save_path=checkpoint_dir, name_prefix='rl_ckpt')

        # evaluation callback (logs to tensorboard dir)
        eval_freq = max(1, int(args.timesteps // 10))
        eval_cb = EvalCallback(eval_env, best_model_save_path=checkpoint_dir, log_path=args.tensorboard_log, eval_freq=eval_freq, deterministic=True, render=False)
        ...
        if args.wandb:
            try:
                ...
                wb_run = wandb.init(project=args.wandb_project, config=vars(args), reinit=True, sync_tensorboard=True)
        ...
        model = PPO(args.policy, vec_env, verbose=1, seed=seed, tensorboard_log=args.tensorboard_log, device=args.device, learning_rate=float(args.learning_rate), n_steps=int(args.n_steps))
