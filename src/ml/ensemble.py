...
class StackedEnsemble:
            ...
            for model_idx, model_name in enumerate(self.model_names):
                ...
                if hasattr(model, 'fit'):
                    ...
                    if hasattr(fitted_model, 'predict_proba'):
                        preds = fitted_model.predict_proba(X_val_fold)[:, 1]
                    else:
                        preds = fitted_model.predict(X_val_fold)
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        for model_idx, model_name in enumerate(self.model_names):
            ...
            fold_predictions = []
            for fitted_model in self.fitted_base_models[model_name]:
                if hasattr(fitted_model, 'predict_proba'):
                    preds = fitted_model.predict_proba(X)[:, 1]
                else:
    ...
    def _update_weights(self) -> None:
        ...
        # Simple weighting: proportional to AUC score
        total_score = sum(self.base_model_scores.values())
        
        if total_score > 0:
            for model_name in self.model_names:
                self.model_weights[model_name] = self.base_model_scores[model_name] / total_score
        else:
