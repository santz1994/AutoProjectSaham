...
def get_price_series_yf(symbol: str, period: str = "1y") -> List[float]:
    ...
    try:
        ...
        if key:
            try:
                from src.pipeline.data_connectors.alpha_vantage import (
                ...
                # raise combined info
                raise RuntimeError(
                    f"yfinance failed; yahoo fallback failed ({e_yahoo}); "
                    f"alphavantage failed ({e_av})"
                )
...
def evaluate_strategy_on_series(
    prices: List[float], strategy_fn: Callable, starting_cash: float = 10000.0
) -> Dict:
...
def score_symbols_for_strategy(
    ...
    for sym in symbols:
        ...
        if allow_demo and isinstance(sym, str) and sym.upper().startswith("DEMO"):
            ...
            metrics = evaluate_strategy_on_series(
                prices, strategy_fn, starting_cash=starting_cash
            )
        ...
        try:
            ...
            if allow_demo:
                ...
                try:
                    ...
                    out[sym] = {
                        "error": (
                            f"no market data available and demo fallback failed: "
                            f"{repr(e)}"
                        ),
                    }
                    continue
