...
def fetch_daily_adjusted(symbol: str, api_key: str, outputsize: str = 'compact', timeout: int = 15) -> List[float]:
    try:
        import requests
    ...
    if ts_key is None:
        ...
        if 'Note' in data:
            raise RuntimeError('AlphaVantage error: ' + data.get('Note', 'rate limit or API issue'))
        if 'Error Message' in data:
            raise RuntimeError('AlphaVantage error: ' + data.get('Error Message', 'unknown'))
