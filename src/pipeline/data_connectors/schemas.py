...
try:
    ...
    try:
        ...
        PYDANTIC_V2 = hasattr(_pydantic, "model_validator")
    except Exception:
        PYDANTIC_V2 = False
    ...
    try:
        ...
        PYDANTIC_V1 = hasattr(_pydantic_v1, "root_validator")
    except Exception:
        PYDANTIC_V1 = False
        ...
        class OHLCVRow(BaseModel):
            ...
            def check_values(self):
                ...
                if getattr(self, "adj_close", None) is not None:
                    try:
                        object.__setattr__(
                            self, "adj_close", float(getattr(self, "adj_close"))
                        )
                    except Exception:
...
def _normalize_row_for_ohlcv(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Returns a dict with keys: date, open, high, low, close, adj_close, volume.
    """
    ...
    return {
        ...
        "adj_close": _fetch(
            ["adj close", "adj_close", "adjusted close", "5. adjusted close"]
        ),
...
def validate_ohlcv_rows(rows: List[Dict[str, Any]]) -> bool:
    ...
    Each row may be a dict-like mapping (as returned by
    ...
    for idx, r in enumerate(rows):
        ...
        try:
            o = norm.get("open")
            h = norm.get("high")
            low_raw = norm.get("low")
            c = norm.get("close")
        except Exception:
            raise ValueError(f"row {idx} missing OHLC columns")

        if o is None or h is None or low_raw is None or c is None:
        ...
        if any(x <= 0.0 for x in (o, h, low, c)):
