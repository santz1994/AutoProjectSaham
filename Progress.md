...
- [-] Penambahan fitur "Remember Me" dan "Two-Factor Authentication" untuk meningkatkan keamanan akun pengguna. (PARTIAL: login kini mendukung rememberMe dengan TTL sesi terpisah; 2FA saat ini baru diterapkan pada kill-switch challenge, belum untuk login akun)
- [-] Implementasi Role-Based Access Control (RBAC) untuk membatasi akses ke fitur tertentu berdasarkan peran pengguna (mis. Developer, admin, trader, viewer). (PARTIAL: role user disimpan pada registry auth dan admin guard sudah diterapkan pada endpoint operasional mutating (bot control, broker connect/disconnect, state migration, broker feature-flag update); RBAC granular per fitur/role matrix masih perlu dituntaskan)
