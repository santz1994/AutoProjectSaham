...
class EndToEndIntegration(unittest.TestCase):
    def test_generate_train_backtest(self):
        ...
        cmd = [sys.executable, os.path.join('bin', 'runner.py'), 'scripts/generate_demo_prices.py', '--', '--symbols', 'BBCA.JK', '--n', '150']
        r = subprocess.run(cmd, check=True)
        self.assertEqual(r.returncode, 0)

        # run backtester (reads data/prices/BBCA.JK.json)
        cmd = [sys.executable, os.path.join('bin', 'runner.py'), 'scripts/test_backtester.py']
        r = subprocess.run(cmd, check=True)
        self.assertEqual(r.returncode, 0)
        ...
        cmd = [sys.executable, os.path.join('bin', 'runner.py'), 'scripts/train_model.py', '--', '--limit', '1', '--out-dataset', out_ds, '--model-out', out_model]
        r = subprocess.run(cmd, check=True)
        self.assertEqual(r.returncode, 0)
