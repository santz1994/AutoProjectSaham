...
class App(tk.Tk):
    def __init__(self):
        ...
        tk.Button(frame, text="Generate Demo Prices", command=self._gen_demo).pack(
            side=tk.LEFT, padx=4
        )
        tk.Button(frame, text="Run Backtester", command=self._run_backtest).pack(
            side=tk.LEFT, padx=4
        )
        tk.Button(frame, text="Train Model", command=self._train_model).pack(
            side=tk.LEFT, padx=4
        )
        tk.Button(frame, text="Run Exec Manager Test", command=self._test_exec).pack(
            side=tk.LEFT, padx=4
        )
        tk.Button(frame, text="Start Metrics Server", command=self._start_metrics).pack(
            side=tk.LEFT, padx=4
        )
    ...
    def _gen_demo(self):
        self._run_script(
            ...
            ["--symbols", "BBCA.JK", "TLKM.JK", "BMRI.JK", "--n", "300"],
        )
